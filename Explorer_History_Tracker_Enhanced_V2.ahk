...
global hGui := 0
global hListView := 0
global reopenIndex := 0  ; Track which item we're reopening with Ctrl+Shift+T
...
; Hotkey to reopen last closed folder (like browser's Ctrl+Shift+T)
^+t::ReopenLastClosed()
...
AddToHistory(path) {
    ...
    ; Reset reopen index when new item is added
    reopenIndex := 0
...
; Reopen last closed folders one by one (like browser Ctrl+Shift+T)
ReopenLastClosed() {
    global closedHistory, reopenIndex
    
    if (closedHistory.Length() = 0) {
        ToolTip, No closed folders in history
    ...
    if (reopenIndex > closedHistory.Length()) {
        reopenIndex := 1
    }
    
    ; Get the folder at current index
    entry := closedHistory[reopenIndex]
    path := entry.path
    
    ; Check if folder is already open in an Explorer window
    existingHwnd := FindExplorerWindow(path)
    
    if (existingHwnd) {
        ; Activate existing window
        WinActivate, ahk_id %existingHwnd%
        ToolTip, Activated existing Item %reopenIndex%: %path%
        SetTimer, RemoveToolTip, -2000
        return
    }
    
    ; Show which item we're reopening
    ToolTip, Reopening Item %reopenIndex%: %path%
    SetTimer, RemoveToolTip, -2000
    ...
    if (FileExist(path)) {
        try {
            ...
            ToolTip, Failed to open: %path%
            SetTimer, RemoveToolTip, -3000
        }
    } else {
        ToolTip, Path no longer exists: %path%
        SetTimer, RemoveToolTip, -3000
    }
}
...
ReopenFolder(path) {
    ...
    if (existingHwnd) {
        ; Activate existing window instead of opening new one
        WinActivate, ahk_id %existingHwnd%
        ToolTip, Activated existing window
        SetTimer, RemoveToolTip, -2000
        return
    }
    ...
    if (FileExist(path)) {
        try {
            ; Force regular Windows Explorer instead of default file manager
            Run, explorer.exe "%path%"
        } catch e {
...
; Find if a path is already open in an Explorer window
FindExplorerWindow(targetPath) {
    try {
        windows := ComObjCreate("Shell.Application").Windows
        Loop % windows.Count {
            window := windows.Item(A_Index - 1)
